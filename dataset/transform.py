...
def normalize(imgA, imgB, mask=None):
    # 归一化处理
...
def blur(imgA, imgB, p=0.5):
    if random.random() < p:
        sigma = np.random.uniform(0.1, 2.0)
        imgA = imgA.filter(ImageFilter.GaussianBlur(radius=sigma))
        imgB = imgB.filter(ImageFilter.GaussianBlur(radius=sigma))
    return imgA, imgB
